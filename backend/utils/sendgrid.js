...
// Function to send a product order email
exports.sendVenueBookedEmail = (order, user) => {
  const dynamicTemplateData = {
    username: user.username,
    orderId: order._id,
    amountPaid: order.totalCost,
  ...
  const message = {
    to: user.email,
    from: process.env.SENDGRID_FROM_EMAIL,
    subject: "Your  venue has been booked",
    dynamicTemplateData: dynamicTemplateData,
    template_id: process.env.SENDGRID_VENUE_BOOK_TEMPLATE_ID,
  };

  sgMail
    .send(message)
    .then(() => console.log("Venue booked"))
    .catch((err) => {
      Promise.reject(err);
    });
};
